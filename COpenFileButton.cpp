...
COpenFileButton::COpenFileButton(QWidget* parent)
		:QWidget(parent)
{
		...
		m_pOpenFileButton->setIcon(QIcon(":/MainWidget/resources/vedioWidget/openfile.svg"));
...
void COpenFileButton::paintEvent(QPaintEvent* event)
{
		...
		painter.setBrush(QBrush(QColor(55, 55, 55)));  // 设置填充颜色
		painter.setPen(Qt::transparent);               // 设置透明边框
		QRect rect = this->rect();
		painter.drawRoundedRect(rect, BUTTON_HEIGHT / 2, BUTTON_HEIGHT / 2); // 绘制圆角矩形，圆角半径为BUTTON_HEIGHT/2（如15px）
